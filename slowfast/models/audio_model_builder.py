...
class AudioMViT(nn.Module):
    ...
    def _construct_network(self, cfg):
        # =============================== Prepare input ===============================
        spatial_size = (cfg.AUDIO_DATA.NUM_FREQUENCIES, cfg.AUDIO_DATA.NUM_FRAMES)  # (256, 256)
        temporal_size = cfg.AUDIO_DATA.NUM_FRAME_CROPS  # 8
        use_2d_patch = False
        self.patch_stride_audio = cfg.MVIT.PATCH_STRIDE

        # =============================== Prepare output ===============================
        num_classes = cfg.MODEL.NUM_CLASSES if len(cfg.MODEL.NUM_CLASSES) > 1 else cfg.MODEL.NUM_CLASSES[0]  # 309
        ...
        self.input_dims_audio = [temporal_size, spatial_size[0], spatial_size[1]]
        self.patch_dims_audio = [self.input_dims_audio[i] // self.patch_stride_audio[i] for i in range(len(self.input_dims_audio))]

        # Positional embedding
        if self.cls_embed_on:
            self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim_audio))  # size (1, 1, 96)
        ...
        if self.cls_embed_on:
            self.pos_embed_class_audio = nn.Parameter(torch.zeros(1, 1, embed_dim_audio))
        ...
        self.norm_audio = norm_layer(dim_out_audio[-1])
        ...
        if self.cls_embed_on:
            trunc_normal_(self.pos_embed_class_audio, std=0.02)
    ...
    def forward(self, x):
        x = self.patch_embed_audio(x)
        T_audio = self.cfg.AUDIO_DATA.NUM_FRAME_CROPS // self.patch_stride_audio[0]  # 4
        H_audio = self.cfg.AUDIO_DATA.NUM_FREQUENCIES // self.patch_stride_audio[1]  # 64
        W_audio = self.cfg.AUDIO_DATA.NUM_FRAMES // self.patch_stride_audio[2]  # 64
        B_audio, N_audio, C_audio = x.shape  # B, 16384, 96

        if self.cls_embed_on:
            cls_tokens = self.cls_token.expand(B_audio, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        ...
        x = self.norm_audio(x)  # x size [B, 256, 768]
