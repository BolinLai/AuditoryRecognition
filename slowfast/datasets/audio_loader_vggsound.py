...
def get_start_end_idx_3d(audio_size, frame_size, frame_interval, num_frames, clip_idx, num_clips):
    clip_size = (num_frames-1) * frame_interval + frame_size
    delta = max(audio_size - clip_size, 0)
    if clip_idx == -1:
        # Random temporal sampling.
        start_idx = random.uniform(0, delta)
    ...
    for i in range(num_frames):
        start_indices.append(start_idx + i * frame_interval)
        end_indices.append(start_idx + i * frame_interval + frame_size - 1)
...
def _log_specgram(cfg, audio, window_size=10,
    ...
    # spectrogram
    spec = stft(audio, n_fft=511,
                window='hann',
                hop_length=noverlap,
                win_length=nperseg,
                pad_mode='constant')

    log_spec = np.log(np.abs(spec) + eps)
    return log_spec.T
...
def _extract_sound_feature_3d(cfg, samples, start_idx, end_idx):
    samples = samples[start_idx:end_idx]
    spectrogram = _log_specgram(cfg, samples,
                                window_size=cfg.AUDIO_DATA.WINDOW_LENGTH,
                                step_size=cfg.AUDIO_DATA.HOP_LENGTH
